...
<%@page import="com.rms.dynamic.data.mapping.form.web.internal.search.DDMFormInstanceRowChecker"%>
<%@page import="com.rms.dynamic.data.mapping.form.web.internal.display.context.helper.FormInstancePermissionCheckerHelper"%>
...
PortletURL portletURL = ddmFormAdminDisplayContexts.getPortletURL();
		 String displayStyles = ddmFormAdminDisplayContexts.getDisplayStyle();

FormInstancePermissionCheckerHelper formInstancePermissionCheckerHelper = ddmFormAdminDisplayContexts.getPermissionCheckerHelper();
%>
...
>
	<aui:form action="<%= portletURL %>" method="post" name="searchContainerForm">
		...
		<c:choose>
			<c:when test="<%= ddmFormAdminDisplayContexts.hasResults() %>">
				<liferay-ui:search-container
					id="<%= ddmFormAdminDisplayContexts.getSearchContainerId() %>"
					rowChecker="<%= new DDMFormInstanceRowChecker(renderResponse) %>"
					searchContainer="<%= ddmFormAdminDisplayContexts.getSearch() %>"
				>
					...
					>
						<portlet:renderURL var="rowURL">
							...
							<portlet:param name="displayStyle" value="<%= displayStyles %>" />
						</portlet:renderURL>
						...
						<c:choose>
							<c:when test='<%= displayStyles.equals("descriptive") %>'>
								...
								<liferay-ui:search-container-column-text>
									<clay:dropdown-actions
										dropdownItems="<%= ddmFormAdminDisplayContexts.getActionDropdownItems(formInstance) %>"
							...
							<c:otherwise>
								...
								<%
								boolean hasValidDDMFormFields = ddmFormAdminDisplayContexts.hasValidDDMFormFields(formInstance);
								boolean hasValidMappedObject = ddmFormAdminDisplayContexts.hasValidMappedObject(formInstance);
								boolean hasValidStorageType = ddmFormAdminDisplayContexts.hasValidStorageType(formInstance);
								%>
									...
									<c:otherwise>
										...
										>
											...
											if (!hasValidDDMFormFields) {
												errorMessage = LanguageUtil.format(request, "this-form-was-created-using-a-custom-field-type-x-that-is-not-available-for-this-liferay-dxp-installation.-instal-x-to-make-it-available-for-editing", ddmFormAdminDisplayContexts.getInvalidDDMFormFieldType(formInstance));
											}
											else if (!hasValidMappedObject) {
												errorMessage = LanguageUtil.format(request, "this-form-was-created-using-an-inactive-object-as-storage-type.-activate-x-object-to-make-it-available-for-editing", ddmFormAdminDisplayContexts.getObjectDefinitionLabel(formInstance, locale));
											}
			...
			<c:otherwise>
				<liferay-frontend:empty-result-message
					actionDropdownItems="<%= ddmFormAdminDisplayContexts.getEmptyResultMessageActionItemsDropdownItems() %>"
					animationType="<%= ddmFormAdminDisplayContexts.getEmptyResultMessageAnimationType() %>"
					buttonCssClass="secondary"
					description="<%= ddmFormAdminDisplayContexts.getEmptyResultMessageDescription() %>"
					title="<%= ddmFormAdminDisplayContexts.getEmptyResultsMessage() %>"
				/>
			</c:otherwise>
		</c:choose>
	</aui:form>
