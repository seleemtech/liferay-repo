    ...
    >
        <div class="add-instance-content">
            <div class="px-4 py-2">
                ...
                <aui:input fieldParam="virtualHostname" label="virtual-host" model="<%= VirtualHost.class %>"
                           name="hostname"/>
                ...
                <c:if test="<%= !siteInitializers.isEmpty() %>">
                    <aui:select label="virtual-instance-initializer" name="siteInitializerKey"
                        ...
                        <aui:option label="<%= siteInitializer.getName(locale) %>"
                                    value="<%= siteInitializer.getKey() %>"/>
                ...
                <c:if test="<%= Validator.isNull(PropsUtil.get(PropsKeys.DEFAULT_ADMIN_PASSWORD)) %>">
                    ...
                    >
                        <aui:input label="field.screen-name" name="defaultAdminScreenName" required="<%= true %>"
                                   type="text"/>

                        <aui:input label="email-address" name="defaultAdminEmailAddress" required="<%= true %>"
                                   type="text"/>
                        ...
                        <c:if test='<%= fullNameDefinition.isFieldRequired("first-name") %>'>
                            <aui:input label="first-name" name="defaultAdminFirstName" required="<%= true %>"
                                       type="text" value="<%= PropsUtil.get(PropsKeys.DEFAULT_ADMIN_FIRST_NAME) %>"/>
                        </c:if>

                        <c:if test='<%= fullNameDefinition.isFieldRequired("middle-name") %>'>
                            <aui:input label="middle-name" name="defaultAdminMiddleName" required="<%= true %>"
                                       type="text" value="<%= PropsUtil.get(PropsKeys.DEFAULT_ADMIN_MIDDLE_NAME) %>"/>
                        </c:if>

                        <c:if test='<%= fullNameDefinition.isFieldRequired("last-name") %>'>
                            <aui:input label="last-name" name="defaultAdminLastName" required="<%= true %>" type="text"
                                       value="<%= PropsUtil.get(PropsKeys.DEFAULT_ADMIN_LAST_NAME) %>"/>
                        </c:if>
                    </liferay-ui:panel>
                </c:if>
            </div>
        ...
        <div class="add-instance-loading align-items-center d-none flex-column justify-content-center">
            ...
            <p class="text-3 text-center text-secondary"><liferay-ui:message
                    key="the-creation-of-the-site-may-take-some-time-.closing-the-window-will-not-cancel-the-process"/></p>
        </div>
